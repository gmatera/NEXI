...
@Slf4j
@Profile("SOAP_OUTBOUND_FACADE")
@Configuration
@PropertySource({ "classpath:/outbound-femws.properties" })
@Import(value = { ConfigCswDomain.class, ConfigCommonService.class, ConfigCswFemws.class, ConfigHttp.class})
@ComponentScan(basePackageClasses = { ConfigCswOutboundFemws.class, HubEncryptionUtil.class })
public class ConfigCswOutboundFemws {

	static {
		ConfigCommon.addMessageResource("i18n/outbound-femws");
	}

	protected ConfigCswOutboundFemws() {
		ServiceRoles.getInstacne().addRole(ServiceRole.SOAP_OUTBOUND_FACADE);
	}

	@PostConstruct
	public void inti() {
		log.info(Color.g(String.format("################# %s profile active", ServiceRole.SOAP_OUTBOUND_FACADE)));
	}
}
