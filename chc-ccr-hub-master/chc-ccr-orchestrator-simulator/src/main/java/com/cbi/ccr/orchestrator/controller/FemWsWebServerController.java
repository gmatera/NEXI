...
@Slf4j
@RestController
@RequestMapping(path = "/webserver/simulator/femws")
public class FemWsWebServerController{
	...
	public void post(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		response.addHeader("x-chc-web-simulator", "web-simulator");
		
		try(ByteArrayInputStream in = new ByteArrayInputStream(out.toByteArray())){
			IOUtils.copy(in, response.getOutputStream());
		}
		
		log.info("WEBSERVER Simulator got Femws message \n {}", out.toString());	
